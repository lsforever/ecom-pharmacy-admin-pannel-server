...
const MedicineType = require('../../models/products/MedicineType')
...
router.delete(
    ...
    async (req, res) => {
        try {
            ...
            let output = await MedicineType.findByIdAndDelete(mongoose.Types.ObjectId(id))
...
router.put(
    ...
    async (req, res) => {
        ...
        try {
            ...
            let output = await MedicineType.findByIdAndUpdate(req.params.id, obj)
            res.status(200).json(output)

        } catch (error) {
            if (error.code === 11000) {
                //Duplicate data
                return res.status(400).json({ message: 'Medicine Type with that name already exists' })
            }
...
router.get('/:id',
    ...
    async (req, res) => {
        try {
            let output = await MedicineType.findById(req.params.id)
            res.status(200).json(output)
...
router.post(
    ...
    [
        check('name', 'name should not be empty')
            .notEmpty(),
        check('com', 'Commission should be a number')
        .optional()
            .isNumeric(),
...
router.get('/',
    ...
    async (req, res) => {
        try {
            ...
            let cat_list = await MedicineType
                .find(filter)
                .sort(sort)
                .skip(range[0])
                .limit(range[1])
            ...
            const count = await MedicineType.countDocuments()
            const header = `product-medicine-type ${range[0] + 1}-${cat_list.length + range[0]}/${count}`
